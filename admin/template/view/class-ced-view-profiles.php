...
class Ced_Etsy_Profile_Table extends WP_List_Table {
	...
	public function get_profiles( $per_page = 10, $page_number = 1 ) {
		...
		$result    = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM  {$wpdb->prefix}ced_etsy_profiles WHERE `shop_name`= %d ORDER BY `id` DESC LIMIT %d OFFSET %d", $shop_name, $per_page, $offset ), 'ARRAY_A' );
		return $result;
